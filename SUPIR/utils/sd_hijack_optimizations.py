...
from SUPIR.utils import devices, shared, sub_quadratic_attention
from SUPIR.utils.sd_hijack_utils import default
...
class SdOptimizationXformers(SdOptimization):
    ...
    def is_available(self):
        return shared.opts.force_enable_xformers or (shared.xformers_available and torch.cuda.is_available() and (6, 0) <= torch.cuda.get_device_capability(shared.device) <= (9, 0))
...
if shared.opts.xformers or shared.opts.force_enable_xformers:
    try:
        import xformers.ops
        shared.xformers_available = True
    except Exception:
        print("Cannot import xformers")
...
def get_xformers_flash_attention_op(q, k, v):
    if not shared.opts.xformers_flash_attention:
        return None
    ...
    try:
        ...
        print(f"enabling flash attention: {e}")
